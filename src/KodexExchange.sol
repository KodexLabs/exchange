...
contract KodexExchange is Ownable, EIP712, Pausable, IKodexExchange {
    ...
    bytes32 public constant _LIST_TYPEHASH =
        keccak256("List(address seller,uint256 tokenId,uint128 askPrice,uint128 duration,uint256 nonce)");
    ...
    constructor(address _ensAddress, address _weth, address _systemFeeWallet, uint96 _systemFeePerStep)
        EIP712(name, version())
    {
    ...
    function list(uint256 _tokenId, uint128 _askPrice, uint128 _duration) external payable whenNotPaused {
    ...
    function listBySig(
        ...
        bytes32 structHash =
            keccak256(abi.encode(_LIST_TYPEHASH, _seller, _tokenId, _askPrice, _duration, _useNonce(_seller)));
    ...
    function offer(uint256 _tokenId, uint128 _offerAmount, uint128 _duration) external payable whenNotPaused {
    ...
    function cancelSignature(address _seller, uint256 _tokenId, uint128 _askPrice, uint128 _duration)
        external
        payable
    {
        bytes32 structHash =
            keccak256(abi.encode(_LIST_TYPEHASH, _seller, _tokenId, _askPrice, _duration, nonces(_seller)));
        bytes32 hash = _hashTypedDataV4(structHash);
    ...
    // solhint-disable-next-line func-name-mixedcase
    function _list(address _seller, uint256 _tokenId, uint128 _askPrice, uint128 _duration) internal {
        // solhint-disable-next-line not-rely-on-time
        require(block.timestamp < _duration, "LISTING_EXPIRED");
    ...
    function _offer(address _offerer, uint256 _tokenId, uint128 _offerAmount, uint128 _duration) internal {
        // solhint-disable-next-line not-rely-on-time
